...
answer. Agreeing starts it and is saved by the app, so that program over that directory is not
asked about again, now or in a later run. Declining starts nothing and is saved nowhere. No
file a project brings can agree, so a shared project asks whoever opens it.

An agreement is for the program and the directory together.
Editing either takes it back and stops the server.
Another project with the same program and directory is already agreed to.
Only the latest fifty agreements are kept.
