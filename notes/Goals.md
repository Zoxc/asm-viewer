...
- [ ] Keep every expensive operation off the UI thread. A standing rule rather than a task that
  ...
  file, and the scratchpad's build and run. Two file reads that had crept into a render and an
...
- [ ] Map a PDB's source paths onto this machine: the names come out verbatim (`C:\...` from
  ...
  checksum (`LineInfo::hash_for`) deciding among candidates rather than the name alone.
- [ ] Check the source hash for DWARF too: carry DWARF 5's `DW_LNCT_MD5` the way the PDB's
  checksum is carried (`LineInfo::hash_for`), so the Source pane's "this file differs from the
