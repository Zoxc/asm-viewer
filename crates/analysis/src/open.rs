...
/// collecting them. A file that fails to read or parse is silently skipped. An archive's
/// members come one member late, so the last can say what was left out: members that stopped
/// early ([`LoadMessage::ArchiveCutShort`]), members that are not objects
/// ([`LoadMessage::UnreadableMembers`]), or a thin archive's, which are not read
/// ([`LoadMessage::ThinArchive`]). An archive with something to say and no member shown is
/// handed over alone, with no format, to say it.
///
...
fn open_one_file(
    ...
    let Ok(archive) = ArchiveFile::parse(file.bytes()) else {
    ...
    match held {
        ...
        None if !said.is_empty() => {
            let object = Object::unread(path.to_path_buf(), name, file, said);
