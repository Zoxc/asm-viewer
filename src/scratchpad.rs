...
/// The process group a run is started in, so that [`Running::stop`] reaches the program's
/// own children as well as the program.
///
/// A scratchpad is a buffer someone is experimenting in, and experimenting with
/// `Command::new` is ordinary; without this, a stop kills the process the app has a handle
/// for and leaves everything it forked running with nothing that could ever find it again
/// — the grandchild's pid was never anywhere but inside the program that is now gone.
/// The two platforms have the same shape and nothing else in common: [`Group::arrange`]
/// runs before the spawn, [`Group::of`] takes hold of what was spawned, and [`Group::kill`]
/// ends the lot.
#[cfg(unix)]
struct Group(i32);
...
#[cfg(windows)]
struct Group(Mutex<Option<std::os::windows::io::OwnedHandle>>);
