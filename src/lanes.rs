...
mod tests {
    ...
    /// The gutter as it is drawn, one string per row, outermost lane on the left — which
    ...
    /// `>` where a branch lands. Trailing blanks are cut, so the strings below are the
    /// picture and not the column the picture sits in.
    ...
    /// Branches that share nothing but a row share a lane's row, which reads as one line
    /// passing through — so they do not share a lane.
    #[test]
    ...
    /// Past the cap the outermost lane is shared, and it is the longest branches that
    /// end up sharing it. Six branches nested one inside the next need six lanes; the
    /// two outermost come out in lane 4 together, and every one of them keeps its corners
    /// and its arrowhead.
    #[test]
    fn more_branches_than_lanes_share_the_outermost_one() {
        ...
        // The two sharing lane 4 are drawn over one another between rows 1 and 10, which
        // is the ambiguity the cap costs -- and both still say where they start, where
        // they land, and that they land there at all.
    ...
    /// rows it actually spans. Its two ends are lit as corners, and nothing between them
    /// is.
    #[test]
