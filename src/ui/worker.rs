...
/// lines inside an answer task. The writing is what is left: freya's
/// [`Writable::write_if`], which hands the judge the state in place under a guard that
/// notifies nobody, and notifies only where the judge says it changed something. So a
/// judge must leave the state as it found it when it answers `false`. It answers whether
/// it wrote, for the caller with something else to do when it did -- a stop that has to
/// tell the worker too.
///
...
/// that has none to give. Nor a copy to judge: what some of these hold *is* the answer --
/// every hit a search found -- and the judge edits it where it is. Where a state has
/// `PartialEq` and the edit is the reader's rather than a worker's, `marks::update` is this
/// same mechanism with the comparison doing the judging.
pub(crate) fn write_if<S: 'static>(state: State<S>, judge: impl FnOnce(&mut S) -> bool) -> bool {
    state.into_writable().write_if(|mut held| judge(&mut held))
}
