...
text. Its filter box takes the same `box_keys` every other box does (`ui/chords.rs`), or the page
that names Ctrl+P would be the one place Ctrl+P typed a `p`.
...
change re-renders only the panes that read it and never the root. Adding or removing a panel is one
...
(`Doors::arrived`), and a tab's ask is spent only once that is the tab on screen. Only the ask does
...
-- a `PlacedAddress` -- and `Stop::in_symbol` a `SectionAddress`, the space a symbol's own
listing draws; `Stop::on` takes the file the line is of, the place itself is private, and
`Stop::place` hands the pair back -- so a line of an object's code is a state nothing can build and no reader of a stop
...
function of what the controller holds and what is remembered (`kept_move`), so the four cases can
be read and tested apart from the effect that spends them. Three things are load-bearing. Reading the controller's position
