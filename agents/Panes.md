...
path can become a symlink to a fifo. So `source::within` opens through `analysis::open_regular`
with `O_NOFOLLOW | O_NONBLOCK`, asks the handle it opened, and reads one byte past the bound at
most.
...
three of the four. The stub is the highlight's own and stays there. A separator and the section
view's `EmptyRow` draw it too, and the caret, where the rows' text starts (`blank_marks`,
`text_left`): each is an empty row the copy takes and the caret can stand on, and a place opened in
an undecoded stretch lands on one. Both marks are
...
answer, the app's own binary (190k stretches) cost 120 ms of UI thread in a debug build and 4 ms in
release for every chunk of 8; laid over, under 1 ms in debug. Counted once on the UI thread, it
still cost 76 ms in debug as the code first appeared, which is why the worker counts it
(`section::stretches_counted` pins both). **Which place, and whether it is kept at
...
the focusable box the keyboard reaches the pane through, the `on_sized` the list's measurement
comes out of, the sweep that carries a run past the edge, and the `VirtualScrollView` itself, one
`code_row_height()` a row. Every line of it is load-bearing -- the one handler the whole
measurement is written from, the padding, the focus a press asks for -- so it is written once and
...
the symbol bar's names, the find bar's buttons, the sidebar's panels, the toolbar, the tab bar and
the pad list and details are `interactive(false)` while a sweep is under way (`use_sweeping`),
since freya's tooltip arms on the hover alone and a pointer dragging a selection past them armed
and showed theirs (`notes/upstream/freya.md`, `a_sweep_held_over_the_sidebar_arms_no_tooltip` and
its two neighbours). Each is gated once, over the box and not per row, chip or button (`PanelBody`,
`Toolbar`, `TabBar`, `PadList`), so a sweep starting draws no row or chip again. The
...
alone went on from the last match after a click, nothing clearing it but a new pattern. Without an
index the run is read by its ends, forward from the far one and back from the near one, as the walk
through an object's code reads it: read at the lead, a word picked out and then found (Ctrl+F clears
the index) was stepped back onto from its end, and the first Shift+Enter went nowhere. A step asked
